...
plot_night2 <- avg.month %>%
  filter(Genus %in% c(
    "Proboscia","Fragilariopsis","Dadabacteriales_uc",
    "Pseudo-nitzschia","Rhizosolenia",
    "Corethron","Tripos","RAD-C_XXX",
    "Dino-Group-I-Clade-1_X","Bacillaria",
    "Dino-Group-II-Clade-7_X","Naviculales",
    "SAR116_clade_uc","Candidatus_Puniceispirillum",
    "Amylibacter","Planktomarina","Polarella",
    "Ascidiaceihabitans","Luteolibacter",
    "Flavobacterium","Chrysophyceae_Clade-C_X",
    "Candidatus_Nitrosopumilus","Magnetospira",
  ...
  mutate(Genus2 = case_when(
    Genus %in% c(
      "Dino-Group-I-Clade-1_X",
      "Dino-Group-II-Clade-7_X")~'Syndiniales',
    Genus %in% c(
      "Chrysophyceae_Clade-C_X","Polarella",
      "Bacillaria","Naviculales") ~ "icetaxa",
    Genus %in% c(
  ...
  scale_y_continuous(
    expand = c(0.01,0.01)) +
  facet_grid(type~biome, scales="free") +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        axis.title = element_blank(),
...
plot_night3 <- avg.month %>%
  ...
  geom_line(aes(
    month, PW, group=1), 
    size=1, colour="gray11", linetype="solid") + 
  geom_point(aes(
    month, PW, 
