...
summer1 <- sum.genus %>% filter(
  site=="WSC" &
    ...
    date != "2017-07-22" & Genus %in% c(
      "OCS116 uc","NS10","NS2b","Aurantivirga",
      "Formosa","Polaribacter","Amylibacter",
      "Cyclobacteriaceae uc","Phaeocystis",
      "Grammonema","Chytriodinium","Chaetoceros",
      "Micromonas","Woloszynskia","Thalassiosira",
      "Odontella")) %>%
  mutate(date = as.Date(
    date, format = "%Y-%m-%d")) %>%
  ungroup %>%
  mutate(Genus = factor(Genus, levels=c(
    "Cyclobacteriaceae uc","Polaribacter",
    "NS10","OCS116 uc","Aurantivirga",
...
summer2 <- sum.genus %>% filter(
  ...
  "Apr","May","Jun","Jul") & !RAS_id %in% c(
    "07_2016_EGC_1","07_2016_EGC_2") &
  Genus %in% c(
    "Colwellia","Aurantivirga",
    "Marinimicrobia_SAR406 uc",
     "Cryomorphaceae uc","Lentimonas",
     "OCS116 uc","Luteolibacter",
